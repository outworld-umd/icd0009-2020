...
namespace DAL.App.EF
{
    public class AppUnitOfWork : EFBaseUnitOfWork<Guid, AppDbContext>, IAppUnitOfWork
    {
        ...
        public INutritionInfoRepository NutritionInfos =>
            GetRepository(() => new NutritionInfoRepository(UOWDbContext));

        public IOrderRepository Orders => GetRepository(() => new OrderRepository(UOWDbContext));

        public IOrderItemChoiceRepository OrderItemChoices =>
            GetRepository(() => new OrderItemChoiceRepository(UOWDbContext));
        ...
        public IRestaurantUserRepository RestaurantUsers =>
            GetRepository(() => new RestaurantUserRepository(UOWDbContext));

        public IRestaurantCategoryRepository RestaurantCategories =>
            GetRepository(() => new RestaurantCategoryRepository(UOWDbContext));
